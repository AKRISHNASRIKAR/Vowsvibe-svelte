<script lang="ts">
	...
	// Track expanded verse
	let expandedVerse: number | null = null;
	...
	function toggleVerse(verseNumber: number) {
		expandedVerse = expandedVerse === verseNumber ? null : verseNumber;
	}
</script>
			...
			{#if loading}
				<div class="flex justify-center items-center py-20">
					<div class="text-center">
						<div
							class="inline-block animate-spin rounded-full h-12 w-12 border-4 border-[#bd003c] border-t-transparent"
						></div>
				...
				<div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 md:gap-6">
					...
					<div class="relative w-full aspect-3/4">
						...
						<div class="absolute inset-0 flex flex-col items-center justify-center">
							<span class="text-white text-xs sm:text-sm font-semibold mb-2 text-center px-2"
								>Whole<br />Chapter</span
							>
							<button
					...
					{#each verses as verse (verse.verse_number)}
						<VerseCard
							{verse}
							isExpanded={expandedVerse === verse.verse_number}
							onToggle={() => toggleVerse(verse.verse_number)}
						/>
					{/each}
					...
					<div class="relative w-full aspect-3/4">
						...
						<div class="absolute inset-0 bg-black/50 rounded-lg"></div>
						<div class="absolute inset-0 flex flex-col items-center justify-center">
							<span class="text-white text-xs sm:text-sm font-semibold text-center px-2"
								>End of<br />Chapter</span
							>
						</div>
					</div>
				</div>
